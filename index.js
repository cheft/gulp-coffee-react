...
var coffeex = require('coffee-react-transform');
...
module.exports = function (opt) {
  ...
  function transform(file, enc, cb) {
    ...
    if (file.isStream()) return cb(new PluginError('gulp-coffee-react', 'Streaming not supported'));
    ...
    try {
      var ccode = coffeex(str);
      data = coffee.compile(ccode, options);
    } catch (err) {
      return cb(new PluginError('gulp-coffee-react', err));
    }
